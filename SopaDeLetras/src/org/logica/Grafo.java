...
import java.util.HashMap; // Necesitarás importar HashMap si no lo tienes
...
public class Grafo {
    ...
    private void conectarVecinos(int fila, int col) {
        ...
        for (int i = -1; i <= 1; i++) {
            for (int j = -1; j <= 1; j++) { // <-- Asegúrate de que este bucle sea de -1 a 1 para todas las direcciones
                if (i == 0 && j == 0) {
                    continue;
                }
    ...
    public ListaSimple<Vertice> buscarPalabraBFS(String palabra) {
        // ... (Tu código BFS actual para encontrar la palabra) ...
        if (palabra == null || palabra.isEmpty()) {
    ...
    public HashMap<Vertice, Vertice> obtenerArbolBFS(Vertice inicio) {
        ...
        // Usamos un array booleano para 'visitado' por simplicidad,
        // pero puedes adaptarlo a tu ListaSimple si lo prefieres,
        // aunque el rendimiento de búsqueda es mejor con un mapa o array.
        boolean[][] visitado = new boolean[FILAS][COLUMNAS];

        cola.encolar(inicio);
        visitado[inicio.getFila()][inicio.getCol()] = true;
        padres.put(inicio, null); // La raíz no tiene padre
        ...
        while (!cola.estaVacia()) {
            ...
            for (int i = 0; i < adyacentes.tamano(); i++) {
                ...
                if (!visitado[vecino.getFila()][vecino.getCol()]) {
                    ...
                    cola.encolar(vecino);
                    padres.put(vecino, actual); // Vecino tiene como padre a 'actual'
                }
            }
        }
        return padres;
